...
//クオータニオンによりベクトルを回転させる
void RotVec(XnPoint3D &v, Quaternion q)
{
	//シグバース初期ベクトルのクオータニオンnによる座標変換
			...
			if(i == 0){
				...
				if(cnt == 0){
					...
					if(DiffVec(khip_vec, rhip, lhip)){
						...
						XnPoint3D krhip_vec;
						//右膝が画面の下に見切れていた場合
						if (rknee.position.Y < -(rknee.position.Z * tan(21.5 / 180. * M_PI))){
							//腰の長さを基準に膝から下の位置を決定
							double hip_distance = sqrt(pow(rhip.position.X - lhip.position.X, 2)
								                     + pow(rhip.position.Y - lhip.position.Y, 2)
													 + pow(rhip.position.Z - lhip.position.Z, 2));
							rknee = rhip;
							rknee.position.Y = rhip.position.Y - hip_distance;
							rfoot = rknee;
							rfoot.position.Y = rknee.position.Y - hip_distance;
						}
						...
						XnPoint3D klhip_vec;
						//左膝が画面の下に見切れていた場合
						if (lknee.position.Y < -(lknee.position.Z * tan(21.5 / 180. * M_PI))){
							//腰の長さを基準に膝から下の位置を決定
							double hip_distance = sqrt(pow(rhip.position.X - lhip.position.X, 2)
								                     + pow(rhip.position.Y - lhip.position.Y, 2)
													 + pow(rhip.position.Z - lhip.position.Z, 2));
							lknee = lhip;
							lknee.position.Y = lhip.position.Y - hip_distance;
							lfoot = lknee;
							lfoot.position.Y = lknee.position.Y - hip_distance;
						}
